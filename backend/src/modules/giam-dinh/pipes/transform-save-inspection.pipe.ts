...
export class TransformSaveInspectionPipe implements PipeTransform {
  transform(value: any) {
    ...
    const inspectionCode = value.inspectionCode || value.inspection_code || '';
    const inspectionDate = value.inspectionDate || value.inspection_date || '';

    // Validate inspection fields
    if (typeof inspectionCode !== 'string' || !inspectionCode.trim()) {
      throw new BadRequestException(
        'inspectionCode must be a non-empty string',
      );
    }

    if (typeof inspectionDate !== 'string' || !inspectionDate.trim()) {
      throw new BadRequestException(
        'inspectionDate must be a non-empty string',
      );
    }

    // Damages are optional when saving a draft.
    if (value.damages !== undefined && !Array.isArray(value.damages)) {
      throw new BadRequestException('damages must be an array');
    }

    const damages = Array.isArray(value.damages)
      ? value.damages.map((damage: any, index: number) => {
          if (!damage || typeof damage !== 'object') {
            throw new BadRequestException(
              `damages[${index}] must be an object`,
            );
          }
          ...
          if (typeof position !== 'string' || !position.trim()) {
            throw new BadRequestException(
              `damages[${index}].damagePosition is required`,
            );
          }

          if (typeof type !== 'string' || !type.trim()) {
            throw new BadRequestException(
              `damages[${index}].damageType is required`,
            );
          }

          if (typeof severity !== 'string' || !severity.trim()) {
            throw new BadRequestException(
              `damages[${index}].severity is required`,
            );
          }

          if (typeof description !== 'string' || !description.trim()) {
            throw new BadRequestException(
              `damages[${index}].description is required`,
            );
          }

          if (typeof method !== 'string' || !method.trim()) {
            throw new BadRequestException(
              `damages[${index}].repairMethod is required`,
            );
          }
          ...
          return {
            damagePosition:
              damage.damagePosition?.trim() ||
              damage.damage_position?.trim() ||
              '',
            damageType:
              damage.damageType?.trim() || damage.damage_type?.trim() || '',
            severity: damage.severity?.trim() || '',
            description: damage.description?.trim() || '',
            repairMethod:
              damage.repairMethod?.trim() || damage.repair_method?.trim() || '',
            images,
          };
        })
      : [];
